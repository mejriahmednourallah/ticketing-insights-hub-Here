...
with fields as (
  ...
  from {{ ref('stg_issues') }}
  union all
  select id, project_name, tracker, 'technology', technology,
  ...
  from {{ ref('stg_issues') }}
  union all
  select id, project_name, tracker, 'nature', nature,
  ...
  from {{ ref('stg_issues') }}
  union all
  select id, project_name, tracker, 'intervention_type', intervention_type,
  ...
  from {{ ref('stg_issues') }}
  union all
  select id, project_name, tracker, 'source', source,
  ...
  from {{ ref('stg_issues') }}
  union all
  select id, project_name, tracker, 'satisfaction', satisfaction,
         {{ json_mapping_value('field_mapping_json', 'satisfaction', 'method') }},
         {{ json_mapping_value('field_mapping_json', 'satisfaction', 'sourcePresent') }},
         {{ json_mapping_value('field_mapping_json', 'satisfaction', 'nonEmptyCandidateCount') }},
         {{ json_mapping_value('field_mapping_json', 'satisfaction', 'conflict') }}
  from {{ ref('stg_issues') }}
)
...
select
  ...
  count(*) filter (where mapped_value <> '') as mapped_count,
  count(*) filter (
    where mapped_value = ''
      and source_present = 'true'
      and cast(coalesce(non_empty_candidates, '0') as integer) = 0
  ) as source_empty_count,
  count(*) filter (
    where mapped_value = ''
      and coalesce(source_present, 'false') = 'false'
  ) as source_absent_count,
  count(*) filter (
    where mapped_value = ''
      and cast(coalesce(non_empty_candidates, '0') as integer) > 0
  ) as mapping_failure_count,
  count(*) filter (where coalesce(has_conflict, 'false') = 'true') as conflict_count,
  round(100.0 * count(*) filter (where mapped_value <> '') / nullif(count(*), 0), 2) as coverage_pct
from fields
group by 1, 2, 3
